...
func initializeFlags() {
	...
	option.BindEnv(option.EnableSCTPName)
	...
	flags.Bool(option.LoadBalancerPreserveWorldID, true, "Set reserved:world security identity for to be forwarded LB requests in tunnel mode")
	...
	flags.Bool(option.EnableRedirectService, false, "Enable google redirect service (beta)")
	option.BindEnv(option.EnableRedirectService)
